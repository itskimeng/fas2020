...
if (isset($month)) {
    ...
    // Now you have $covered_period and $quarter set for each month.
    // Insert your code logic here.


    
    // Now you can use $covered_period and $quarter for each month.
    // Insert your code logic here.


} else {
  $covered_period = $quarter;


  if ($covered_period == '1' || $covered_period == 0) {
    $covered_period = "('1','2','3')";
    $quarter = '1st Quarter';
  } else if ($covered_period == '2') {
    $covered_period = "('4','5','6')";
    $quarter = '2nd Quarter';
  } else if ($covered_period == '3') {
    $covered_period = "('7','8','9')";
    $quarter = '3';
  } else if ($covered_period == '4') {
    $covered_period = "('10','11','12')";
    $quarter = '4th Quarter';
  }
}
...
$sql_q10 = mysqli_query($conn, "SELECT MONTHNAME(`REQ_DATE`) AS 'month', YEAR(`REQ_DATE`) AS 'year', `ID`, `CONTROL_NO`, `REQ_DATE`, `REQ_TIME`,emp.FIRST_M, emp.LAST_M, `REQ_BY`, `OFFICE`, `POSITION`, `CONTACT_NO`, `EMAIL_ADD`, `EQUIPMENT_TYPE`, `BRAND_MODEL`, `PROPERTY_NO`, `SERIAL_NO`, `IP_ADDRESS`, `MAC_ADDRESS`, `TYPE_REQ`, `TYPE_REQ_DESC`, `ISSUE_PROBLEM`, `START_DATE`, `START_TIME`, `STATUS_DESC`, `COMPLETED_DATE`, `COMPLETED_TIME`, `ASSIST_BY`, `PERSON_ASSISTED`, `TIMELINESS`, `QUALITY`, tbltechnical_assistance.`STATUS` FROM `tbltechnical_assistance`  LEFT JOIN tblemployeeinfo emp on tbltechnical_assistance.REQ_BY = emp.EMP_N WHERE MONTH(REQ_DATE) IN $covered_period and YEAR(REQ_DATE) = $year and `REQ_DATE` != '0000-00-00'   order by  `CONTROL_NO`");
...
if (mysqli_num_rows($sql_q10) > 0) {
  ...
  while ($excelrow = mysqli_fetch_assoc($sql_q10)) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $row, $excelrow['FIRST_M'] . ' ' . $excelrow['LAST_M']);
    ...
    if (strlen($excelrow['ISSUE_PROBLEM']) >= 10) {
      $objPHPExcel->getActiveSheet(0)->getRowDimension($row)->setRowHeight(30);
      $objPHPExcel->getActiveSheet(0)->getStyle('G' . $row)->getAlignment()->setWrapText(true);
    } else {
      $objPHPExcel->getActiveSheet(2)->getRowDimension($row)->setRowHeight(14.40);
      $objPHPExcel->getActiveSheet(2)->getStyle('G' . $row)->getAlignment()->setWrapText(true);
    }
