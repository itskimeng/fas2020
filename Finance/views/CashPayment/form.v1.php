...
<script type="text/javascript">
  ...
  function generateUACSItems($data) {
   ...
   function generateNTAItems($data) {
    $.each($data, function(key, item){
      ...
      tr += '<input name="obid[]" type="hidden" value="'+item.obligation_id+'">';
  ...
  function calculate_values()
  {
    ...
    var x_total_dv_deduction = format_number(total_dv_deduction);
    $('.total_dv_deduction').val(x_total_dv_deduction);
    $('.x_total_dv_deduction').val(total_dv_deduction);
    
    $('.p_net_amount').each(function(){
        total_net_amount += parseFloat(this.value);
    });

    var x_total_net_amount = format_number(total_net_amount);
    $('.total_dv_net').val(x_total_net_amount);
    ...
    var x_total_ob_amount = format_number(total_ob_amount);
    $('.total_ob_amount').val(x_total_ob_amount);
    $('.x_total_ob_amount').val(total_ob_amount);
    
    $('.p_nta_amount').each(function(){
        total_nta_amount += parseFloat(this.value);
    });

    var x_total_nta_amount = format_number(total_nta_amount);
    $('.total_nta_amount').val(x_total_nta_amount);
    $('.x_total_nta_amount').val(total_nta_amount);
    
    $('.p_nta_balance').each(function(){
        total_nta_balance += parseFloat(this.value);
    });

    var x_total_nta_balance = format_number(total_nta_balance);
    $('.total_nta_balance').val(x_total_nta_balance);
    $('.x_total_nta_balance').val(total_nta_balance);
    
    $('.p_nta_disbursed_amount').each(function(){
        total_disbursed_amount += parseFloat(this.value);
    });

    var x_total_disbursed_amount = format_number(total_disbursed_amount);
    $('.total_disbursed_amount').val(x_total_disbursed_amount);
    $('.x_total_disbursed_amount').val(total_disbursed_amount);
