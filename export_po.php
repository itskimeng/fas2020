...
while ($excelrow = mysqli_fetch_assoc($sql_items)) {
  ...
  if(strlen($desc) > 50)
  {
    $objPHPExcel->getActiveSheet()->getRowDimension($row)->setRowHeight(63);
  }else{
...
$objPHPExcel->getActiveSheet()->getStyle('A48')->getFont()->setItalic(true);
$objPHPExcel->getActiveSheet()->getStyle('A48')->getFont()->setBold(true);
$objPHPExcel->setActiveSheetIndex()->setCellValue('A48','(Total Amount in Words)   ' . numberTowords($sumtotal) . ' pesos only');
// $objPHPExcel->getActiveSheet()->getRowDimension($row)->setRowHeight(30);
// $objPHPExcel->getActiveSheet()->getStyle('A' . $row)->applyFromArray($border);
// $objPHPExcel->getActiveSheet()->getStyle('B' . $row)->applyFromArray($border);
// $objPHPExcel->getActiveSheet()->getStyle('C' . $row)->applyFromArray($border);
