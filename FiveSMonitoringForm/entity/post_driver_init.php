...
if (isset($_POST['submit'])) {
    ...
    if (empty($pid)) {
        $parent = $fives->insert2($conn, ['emp_id'=>$userid, 'status'=>'Draft', 'date_created'=>$today->format('Y-m-d H::i:s'), 'subtotal'=>$subtotal]); 
    } else {
        $is_new = false;
        $fives->update($conn, ['id'=>$pid, 'total'=>$subtotal]); 
        $parent = $pid;
    }
    ...
    header('location:../../base_fives_driver_edit_form.html.php?username='.$_SESSION["username"].'&division='.$_SESSION["division"].''.'&parent='.$parent.'');
