<div class="row">
    <div class="col-xs-12 col-md-12">
        <table class="table table-bordered table-responsive">
            <tbody>
                <tr>
                    <td><h4>S1 - Sort - SEIRI: <b class="pull-right" style="font-family: Adobe Arabic; font-size: 17pt;"><i>"Determine what is needed and remove the rest"</i></b></h4></td>
                </tr>
            </tbody>
        </table>
    </div>
...
<form method="POST" action="FiveSMonitoringForm/entity/post_sort.php">
    <div class="row">
        <div class="col-xs-12 col-md-12">
            ...
            <table id="list_table" class="table table-striped table-bordered table-responsive table-hover" role="grid">
              ...
              <tbody id="tbody-sort">
                <?php if (!empty($pid)): ?>
                    <?php foreach ($fetchData['S1'] as $key => $data): ?>
                        <tr>
                            <td>
                                <?php echo input_hidden('hidden_id','hidden_id[]','hidden_id',$data['id']); ?>
                                <?php echo input_hidden('hidden_entid','hidden_entid['.$data['id'].']','hidden_entid',!empty($fetchEditFormData['S1'][$data['id']]['id']) ? $fetchEditFormData['S1'][$data['id']]['id'] : ''); ?>
                                <b><?php echo $key+1; ?>.</b> <?php echo $data['particulars']; ?>
                            </td>
                            <td>
                                <?php if ($form_submitted AND $fetchEditFormData['S1'][$data['id']]['score'] == 1): ?> 
                                    <div class="row">
                                        <i class="fa fa-check-square" style="font-size: 19pt;margin-left: 38%; margin-top: 10%; color: #e74040;"></i>
                                    </div>
                                    <?php else: ?>
                                        <?php echo group_input_checkbox_minimal('One', 'one', 'scores['.$data['id'].'][]', 'rating one', 'one', 0, '', $fetchEditFormData['S1'][$data['id']]['score'] == 1 ? true:false, $form_submitted ? true:false); ?>
                                    <?php endif ?>
                                </td>
                                <td>
                                    <?php if ($form_submitted AND $fetchEditFormData['S1'][$data['id']]['score'] == 2): ?> 
                                        <div class="row">
                                            <i class="fa fa-check-square" style="font-size: 19pt;margin-left: 38%; margin-top: 10%; color: #e74040;"></i>
                                        </div>
                                        <?php else: ?>
                                            <?php echo group_input_checkbox_minimal('Two', 'two', 'scores['.$data['id'].'][]', 'rating two', 'two', 0, '', $fetchEditFormData['S1'][$data['id']]['score'] == 2 ? true:false, $form_submitted ? true:false); ?>
                                        <?php endif ?>

                                    </td>
                                    <td>
                                        <?php if ($form_submitted AND $fetchEditFormData['S1'][$data['id']]['score'] == 3): ?> 
                                            <div class="row">
                                                <i class="fa fa-check-square" style="font-size: 19pt;margin-left: 38%; margin-top: 10%; color: #e74040;"></i>
                                            </div>
                                            <?php else: ?>
                                                <?php echo group_input_checkbox_minimal('Three', 'three', 'scores['.$data['id'].'][]', 'rating three', 'three', 0, '', $fetchEditFormData['S1'][$data['id']]['score'] == 3 ? true:false, $form_submitted ? true:false); ?>
                                            <?php endif ?>
                                        </td>
                                        <td>
                                            <?php if ($form_submitted AND $fetchEditFormData['S1'][$data['id']]['score'] == 4): ?> 
                                                <div class="row">
                                                    <i class="fa fa-check-square" style="font-size: 19pt;margin-left: 38%; margin-top: 10%; color: #e74040;"></i>
                                                </div>
                                                <?php else: ?>
                                                    <?php echo group_input_checkbox_minimal('Four', 'four', 'scores['.$data['id'].'][]', 'rating four', 'four', 0, '', $fetchEditFormData['S1'][$data['id']]['score'] == 4 ? true:false, $form_submitted ? true:false); ?>
                                                <?php endif ?>

                                            </td>
                                            <td>
                                                <?php if ($form_submitted AND $fetchEditFormData['S1'][$data['id']]['score'] == 5): ?> 
                                                    <div class="row">
                                                        <i class="fa fa-check-square" style="font-size: 19pt;margin-left: 38%; margin-top: 10%; color: #e74040;"></i>
                                                    </div>
                                                    <?php else: ?>
                                                        <?php echo group_input_checkbox_minimal('Five', 'five', 'scores['.$data['id'].'][]', 'rating five', 'five', 0, '', $fetchEditFormData['S1'][$data['id']]['score'] == 5 ? true:false, $form_submitted ? true:false); ?>
                                                    <?php endif ?>
                                                    
                                                </td>
                                                <td>
                                                    <?php echo group_textarea('Comments','comments['.$data['id'].']', $fetchEditFormData['S1'][$data['id']]['comments'], 0, false, $form_submitted ? true:false); ?>
                                                </td>
                                            </tr>    
                                        <?php endforeach ?>   
                                        <?php else: ?>
                                    ...
                                    <tfoot style="background-color: lightslategray; color:white;">
                                        <tr>
                                            <td>
                                                <?php echo input_hidden('seiri_subtotal','seiri_subtotal','seiri_subtotal', $fetchData1['seiri_subtotal']); ?>
                                                <b class="pull-right">Subtotal</b>
                                            </td>
                                            <td colspan="5" class="subtotal_field" style="text-align: center; font-size: 15pt; font-weight: bold;">
                                                <b><?php echo $fetchData1['seiri_subtotal']; ?></b>
                                            </td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
