...
$genpdf = generatePDF($conn, $user, $pdf, $data);
...
function generatePDF($conn, $user, $pdf, $data)
{
	error_reporting(~0);
...
function fetchData($conn, $activity, $task, $collaborators) 
{
	$array = $collaborators;
	...
	if (!empty($activity)) {
		$sql.= " AND e.title LIKE '%".$activity."%' ";
	}

	if (!empty($task)) {
		$sql.= " AND es.title LIKE '%".$task."%' ";
	}
