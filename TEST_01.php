...
class SSP {
     ...
     *
     */
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '',$extraWhere1='', $groupBy = '', $having = '')
    {
        ...
        if($extraWhere && $extraWhere1)
            $extraWhere = ($where) ? ' AND '.$extraWhere. ' AND '.$extraWhere1 : ' WHERE '.$extraWhere.' AND '.$extraWhere1;
        ...
        if($joinQuery){
           ...
           // if(preg_match( '/^index.php*/', $var1[3]) ) {
            array_push($col,"`d`.`DIVISION_M` AS nme","`e`.`DIVISION_M` as nmf");
            // }
            $col = SSP::pluck($columns, 'db', $joinQuery);
            // array_push($col,"`u`.`name` as nmh");
