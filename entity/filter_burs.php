...
function filterApplicants($conn,$options)
{
        ...
        $sql = "SELECT id,received_by,date,datereceived, datereprocessed, datereturned, datereleased, burs, ponum, payee, particular, sum(amount) as amount  , remarks, reason, sarogroup, status, dvstatus FROM saroobburs WHERE  ";
         if($options['payee'] != '')
        {
            $sql.= " payee = '".$options['payee']."'"; 


        }
        else if (!empty($options['burs'])) {
            $sql.= " burs = '".$options['burs']."'"; 
            ...
            if($options['ors_date'] == '1970-01-01')
            {
        ...
        $sql .= "  group by burs desc limit 0,10";
